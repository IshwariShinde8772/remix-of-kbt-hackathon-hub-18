...
serve(async (req) => {
  ...
  try {
    ...
    const { data: existingTeams } = await primarySupabase
    ...
    if (existingTeams && existingTeams.length > 0) {
      return new Response(
        JSON.stringify({ error: `This team is already registered with ID: ${existingTeams[0].team_id}` }),
    ...
    // ── Return success IMMEDIATELY, then send email & sync in background ──
...
<body style="margin:0;padding:0;background-color:#f4f4f7;font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f4f4f7;padding:20px 0;">
          ...
          <tr>
            <td style="padding:40px 35px;">
              ...
              <p style="color:#475569;font-size:16px;line-height:1.6;margin:0 0 25px;">
                Your team <strong style="color:#0f172a;">"${data.team_name}"</strong> has been successfully registered for <strong>KBT Avinyathon 2026</strong>.
              </p>
    ...
    // Fire background work without awaiting (email + sync don't block response)
    backgroundWork().catch(err => console.error("Background work error:", err));
