...
serve(async (req) => {
  ...
  try {
    ...
    if (existing && existing.length > 0) {
      return new Response(
        JSON.stringify({ error: `This team is already registered. Your Team ID: ${existing[0].registration_id}` }),
    ...
    const { data: result, error: insertError } = await supabase
      ...
      .insert({
        ...
        status: "registered",
        // registration_id is auto-set by the DB trigger → KBT-0001, KBT-0002, ...
      })
