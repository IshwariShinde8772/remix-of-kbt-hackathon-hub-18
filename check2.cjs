...
async function check() {
  const { data: pData } = await supabase.from('problem_statements').select('id, problem_title').eq('id', 'ab10f4c9-a556-4ab1-a809-4b1cd53dd5ea');
  console.log('Problem Match:', pData);
}
check();
